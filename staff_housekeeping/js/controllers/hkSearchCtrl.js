hkRover.controller('HKSearchCtrl', ['$scope', '$rootScope', 'HKSearchSrv', '$state', '$timeout', 'fetchedRoomList',
function($scope, $rootScope, HKSearchSrv, $state, $timeout, fetchedRoomList) {
	...
	var fetchFloors = function() {
		...
		$scope.$emit('showLoader');
		HKSearchSrv.fetch_floors().then(function(data) {
			$scope.$emit('hideLoader');
			$scope.floors = data;
		}, function() {
			console.log("fetch failed");
	...
	angular.element(roomsEl).bind('ontouchmove', function(e) {
		e.stopPropagation();
	});

	// stop browser bounce while swiping on filter-options element
	angular.element(filterOptionsEl).bind('ontouchmove', function(e) {
		e.stopPropagation();
	});
	...
	var pullRefresh = function() {
		...
		var $rooms = document.getElementById('rooms'), $notify = document.getElementById('pull-refresh-notify'), $arrow = document.getElementById('icon'), $notifyTxt = document.getElementById('ref-text');

		// flags and variables necessary
		var touching = false, startY = 0, nowY = 0, initTop = $rooms.scrollTop, trigger = 100;
