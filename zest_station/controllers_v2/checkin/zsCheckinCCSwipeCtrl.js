sntZestStation.controller('zsCheckinCCSwipeCtrl', [
    ...
    function($scope, $stateParams, $state, zsEventConstants, $controller, $timeout, zsCheckinSrv, zsModeConstants, zsGeneralSrv, zsPaymentSrv, $log) {
        ...
        var isEmvEnabled = function() {
            ...
            if (paymentGateway === 'sixpayments' ||
               (paymentGateway === 'MLI' && $scope.zestStationData.mliEmvEnabled) ||
                paymentGateway === 'SHIJI' ||
                (!$scope.zestStationData.is_standalone &&
                $scope.zestStationData.hotelSettings.enable_emv_for_overlay)) {
                return true;
            }
            return false;
        };
