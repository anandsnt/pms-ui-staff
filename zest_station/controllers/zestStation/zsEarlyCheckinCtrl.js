sntZestStation.controller('zsEarlyCheckinCtrl', [
    ...
    function ($scope, $state, zsModeConstants, zsEventConstants, zsTabletSrv, zsPaymentSrv, zsUtilitySrv, $stateParams, $sce, $timeout) {
        ...
        $scope.init = function () {
            ...
            if (current === 'zest_station.early_checkin_prepaid') {
                $scope.is_early_prepaid = $scope.zestStationData.is_early_prepaid;
                $scope.reservation_in_early_checkin_window = $scope.zestStationData.reservation_in_early_checkin_window;
            }
        ...
        $scope.beginEarlyCheckin = function (response) {
            $scope.zestStationData.reservation_in_early_checkin_window = response.reservation_in_early_checkin_window;
            ...
            if (!response.is_room_ready || !response.is_room_already_assigned) {
                ...
                console.info(':: response.early_checkin_offer_id :: ', response.early_checkin_offer_id)
                var eciResponseData = {'early_checkin_offer_id': response.early_checkin_offer_id};//pass to stateParams for other states to use

                if ($scope.reservationIncludesEarlyCheckin(response)) {
                    $scope.selectedReservation.earlyCheckinCharge = response.early_checkin_charge;
                    $scope.early_checkin_offer_id = response.early_checkin_offer_id;
                    //$state.early_checkin_offer_id = response.early_checkin_offer_id;
                    $state.go('zest_station.early_checkin_prepaid', eciResponseData);
                    ...
                    if (inUpsellWindow && response.early_checkin_charge !== null) {
                        ...
                        if (response.is_early_checkin_bundled_by_addon) {
                            ...
                            $state.go('zest_station.early_checkin_nav', eciResponseData);
                        ...
                        $state.go('zest_station.early_checkin_unavailable', eciResponseData);
                    }
                }
            }
        };
        ...
        $scope.acceptEarlyCheckinOffer = function () {
            ...
            $scope.callAPI(zsPaymentSrv.acceptEarlyCheckinOffer, {
                params: {
                    reservation_id: $scope.selectedReservation.id,
                    early_checkin_offer_id: $stateParams.early_checkin_offer_id
                },
