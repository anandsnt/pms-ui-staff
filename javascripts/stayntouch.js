...
function styleInputFile() {
	if ($('input.file, input[type="file"]').length) {
		var $wrapper = '<div class="file-upload" />',
			$fauxLabel = '<span class="input r5">No file chosen</span>',
			$fauxReplaceLabel = '<span class="input r5">Replace</span>',
			$fauxButton = '<a href="#" class="button r5">Choose file</a>';

		$('input.file, input[type="file"]').each(function(){
			var $parent = $(this).parent();

			if ($parent.hasClass('has-file')) { var $label = $fauxReplaceLabel; }
			else { var $label = $fauxLabel; }
...
function styleSelect() {
	if ($('select.select').length) {
		$('select.select').each(function(){
			...
			$(this).wrap('<div class="selectify ' + $(this).attr('class') + '" />')
				.css({'z-index':10,'opacity':0,'-khtml-appearance':'none'})
				.after('<span class="select r5">' + $title + '</span><span class="arrow" />')
				.change(function(){
	...
	if ($('select.timepicker').length) {
		$('select.timepicker').each(function(){
			...
			$(this).wrap('<div class="numberify ' + $(this).attr('class') + '" />')
				.css({'z-index':10,'opacity':0,'-khtml-appearance':'none'})
				.after('<span class="select r5">' + $title + '</span><span class="double-arrow" />')
				.change(function(){
					val = $('option:selected',this).text();
					$(this).next().text(val).addClass('selected');
...
function checkHeight() {
	$('.content-form label:not(.switcher)').each(function(){
		...
		if ($maxHeight > 35)  $(this).addClass('tworow');
	});
	...
	$('.listing-item:not(.singleton), .listing-item:not(.tworow)').each(function(){
		$(this).children('.data').find('p').removeAttr('style').attr({'style': 'height:' + $(this).height() + 'px;'});
		$(this).children('.delete').attr({'style': 'height:' + $(this).height() + 'px;'});
	});
}
...
function addPermission(role, permission) {
	$.ajax({
		type : 'post',
		url : '/role_permissions/add_permission',
		data : 'role_id=' + role + '&permission_id=' + permision,
		dataType : 'json',
		success : function() {
		},
		failure : function(data) {
...
function removePermission(role, permission) {
	$.ajax({
		type : 'post',
		url : '/role_permissions/remove_permission',
...
jQuery(function($){ 
	...
	$('#wrapper').css('opacity','0').delay(200).animate({opacity:1},400);
	...
	$('#page-nav li').each(function(){
		...
		if ($maxHeight > 60)  $(this).addClass('tworow');
	});

	// Check if form has some values ready
	$('.form input').each(function(){
		var $attr = $(this).attr('value');
		if (typeof $attr !== 'undefined' && $attr !== false && $attr != '')  $(this).addClass('has-content');
	});
	...
	$('.has-password').each(function(){
		if ($(this).children('.toggle-password').text() == 'Hide') 
			$(this).children('input.is-password').attr({'type':'text'});

		$(this).children('.toggle-password').click(function(){
			$(this)
				.text($(this).text() == 'Show' ? 'Hide' : 'Show')
				.attr('class', $(this).attr('class') == 'toggle-password stars' ? 'toggle-password text' : 'toggle-password stars')
				.prev('input.is-password').attr('type', $(this).prev('input.is-password').attr('type') == 'text' ? 'password' : 'text');
		});
	});
	...
	$('.preview-toggle').click(function(e){
		...
		switch(this.id){
			...
			case "promotion-lobby":
				var $person = $('#user_id option[value!=0]:selected').text(), 
				$position = $('#user_id option[value!=0]:selected').attr('data-position'),
				 $hotel = $('#user_id option[value!=0]:selected').attr('data-hotel'), 
				 $text = $('#promotion-description').val();
            ...
            case "group-event":
				var $title = $('#event-name').val(),
					...
					$start = $('#event-start').val().split(/ +/),
					$end = $('#event-end').val().split(/ +/);
...
/* Plugins ***/
    ...
    $('#permissions-list').tabs({
    	...
    	activate: function(event, ui){
    		var $active = $(this).tabs('option', 'active'),
    			$visible = $('.ui-tabs-nav > li a').eq($active).attr('href')
	...
	$('.ui-tabs-panel > .cols3 > .sortable li').click(function(){
		...
		if ($selected > 0)
		{
			if ($toAvailable) $('#to-available').addClass('active');
			else if ($toAssigned) $('#to-assigned').addClass('active');
		}
		else 
		{
			if ($toAvailable) $('#to-available').removeClass('active');
			else if ($toAssigned) $('#to-assigned').removeClass('active');
		}
	});
		...
		.sortable({ 
			...
			receive: function( event, ui ) {
				...
				var $selectedAvailable = $('.available .sortable li.selected').length,
				    ...
				    entity_id = ui.item.attr('id').split('_');
				if ($selectedAssigned = 0) $('#to-available.mover').removeClass('active');
				if ($selectedAvailable = 0) $('#to-assigned.mover').removeClass('active');
				if ($toAvailable) {
					addPermission(role_id[1], entity_id[1] + '_' + entity_id[2]);
				} else if ($toAssigned) {
					removePermission(role_id[1], entity_id[1] + '_' + entity_id[2]);
				}
			...
			switch(this.id){
				case "to-available":
					...
					$('.assigned').find('.selected').each(function() {
						...
						entity_id[i++] = arr[1] + '_' + arr[2];
					})
					removePermission(role_id[1], entity_id);
				break; 
	            case "to-assigned":
					...
					addPermission(role_id[1], entity_id);
	            break;
	            default:
	            	return false;
	            break;
