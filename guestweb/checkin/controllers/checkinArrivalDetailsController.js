(function() {
	...
	$scope.postStayDetails = function(){
		...
		preCheckinSrv.postStayDetails(dataTosend).then(function(response) {			
			...
			if(response.early_checkin_available && typeof response.early_checkin_offer_id !== "undefined" && !response.is_eligible_for_free_early_checkin){
				...
				else if(response.early_checkin_on && !response.early_checkin_available && !response.is_eligible_for_free_early_checkin){
