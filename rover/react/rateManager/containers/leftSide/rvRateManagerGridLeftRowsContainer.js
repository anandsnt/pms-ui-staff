...
let convertRateTypesDataForLeftListing = (rateTypes, mode) => {
	...
	rateTypes.map((rateType, index) => {
		rateTypesToReturn.push({
			...
			tdClassName: '',
			// leftSpanClassName: 'name ' + (rateType.based_on_rate_id && !rate.is_copied ? 'gray' : 'base-rate')+((rate.is_company_card||rate.is_travel_agent)?' contracted-rate':' contracted-rate contracted-rate-missing-info'),
			// showIconBeforeText: !rate.based_on_rate_id,
