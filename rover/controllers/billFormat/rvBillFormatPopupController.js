sntRover.controller('rvBillFormatPopupCtrl', ['$scope', '$rootScope', '$filter', 'RVBillCardSrv', 'RVContactInfoSrv', 'ngDialog', function($scope, $rootScope, $filter, RVBillCardSrv, RVContactInfoSrv, ngDialog) {
    ...
    */
    ...
    $scope.getPrintButtonClass = function() {
        ...
        if (!$scope.isInformationalInvoice && (parseInt($scope.reservationBillData.bills[$scope.currentActiveBill].print_counter) >= parseInt($scope.reservationBillData.no_of_original_invoices) && $scope.roverObj.noReprintReEmailInvoice && parseInt($scope.reservationBillData.no_of_original_invoices) !== 0)) {
    ...
    $scope.getEmailButtonClass = function() {
        ...
        } else if (!$scope.isInformationalInvoice && (parseInt($scope.reservationBillData.bills[$scope.currentActiveBill].email_counter) >= parseInt($scope.reservationBillData.no_of_original_emails) && $scope.roverObj.noReprintReEmailInvoice && parseInt($scope.reservationBillData.no_of_original_invoices) !== 0)) {
            emailButtonClass = "grey";
        }
