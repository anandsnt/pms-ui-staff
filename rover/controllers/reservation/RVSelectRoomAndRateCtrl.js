sntRover.controller('RVSelectRoomAndRateCtrl', [
	...
	function($rootScope, $scope, areReservationAddonsAvailable, $stateParams, rates, ratesMeta, $timeout, $state, RVReservationBaseSearchSrv, RVReservationStateService, RVReservationDataService, house, RVSelectRoomRateSrv, rvPermissionSrv, ngDialog, rateAddons, $filter, RVRoomRatesSrv) {
		...
		var getScrollerObject = function(key) {
			...
			generateRatesGrid = function(ratesSet, append) {
				...
				_.each(ratesSet, function(rate) {
					var proccesedRestrictions = processRestrictions(rate.first_restriction, rate.multiple_restrictions),
						rateInfo = {
							isCollapsed: true,
		...
		var processRestrictions = function(firstRestriction, hasMultipleRestrictions) {
			...
			if (!room.totalRatesCount || showMoreRates) {
				...
				fetchRatesList(room.id, null, pageToFetch, function(response) {
					...
					_.each(response.results, function(rate) {
						...
						proccesedRestrictions = processRestrictions(rate.first_restriction, rate.multiple_restrictions);
