...
.controller('rvDiaryCtrl', 
	...
	/*Initialization of React/Angular hooks and callbacks*/
		...
		/*
			calculate if any room should be edit by default
			save the grid value
		*/
...
/*	    	var filter = _.extend({}, $scope.gridProps.filter);
		...
		$scope.setAccountID = function(index) {
			if(index < $scope.companyCardResults.length)
				$scope.gridProps.filter.account_id = $scope.companyCardResults[index].id;
			else
