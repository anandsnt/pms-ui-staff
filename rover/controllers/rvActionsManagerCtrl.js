sntRover.controller('RVActionsManagerController', ['$scope', '$filter', '$rootScope', 'ngDialog', 'rvActionTasksSrv', 'RVReservationCardSrv','hotelDetails', '$state', '$stateParams',
	...
	$scope.setUpData = function() {
        ...
        $scope.isStandAloneAction = function(action){
            ...
            if (!$scope.isTrace(action.action_task_type) && 
         ...
         */
         ...
         $scope.$watch('filter_department',function(to){
             $scope.setActiveFilter($scope.activeFilter);//re-applies the current filter on-top of filtering by department
                setTimeout(function(){
                if (to){
                    var department = to.name;
                    $scope.filterDepartment = department;
                    for (var i in $scope.actions){
                        if (!$scope.actions[i].assigned_to || $scope.actions[i].assigned_to.name !== department){
                            if ($scope.actions[i].assigned_to){
                                //console.log($scope.actions[i].assigned_to.name);
                            }
                            $scope.actions[i].is_deleted = true;
                        }
                    }
                    $scope.$digest();
               }
        ...
        angular.forEach($scope.hotelDetails.userHotelsData.hotel_list, function(hotel, index) {
            ...
            $scope.setActiveFilter = function(filter){
                $scope.actions = [];
                angular.copy($scope.lastFetchActionsList, $scope.actions);
                ...
                if (!$scope.showExtraFilters || $scope.activeFilter === 'All'){
                    return;
                }
                
                //otherwise filter down the list
                for (var i in $scope.actions){
                    if ($scope.actions[i].action_status.toUpperCase() !== $scope.activeFilter.toUpperCase()){
                        $scope.actions[i].is_deleted = true;
                    }
                }
                refreshScroller();
